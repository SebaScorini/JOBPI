...
export function DashboardPage() {
  ...
  const motionListVariants: Variants = allowRichMotion
    ? containerVariants
    : {
        hidden: { opacity: 0 },
        show: { opacity: 1 },
  ...
  const motionItemVariants: Variants = allowRichMotion
    ? itemVariants
    : {
        hidden: { opacity: 0 },
        show: { opacity: 1 },
    ...
    <AnimatePresence mode="wait">
      {isLoading ? (
        ...
        <motion.div 
          ...
          className="space-y-4"
        >
          <motion.header variants={motionItemVariants} className="mb-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] gap-4">
            <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-4">
              ...
              <motion.div variants={motionItemVariants} className="glass-card p-5 rounded-2xl border-brand-primary/20 dark:border-brand-primary/20 flex flex-col justify-center items-start bg-brand-primary/5">
                ...
                <MotionLink 
                  whileHover={allowRichMotion ? { scale: 1.02 } : undefined}
